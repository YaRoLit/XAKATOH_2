...
''''
Варианты препроцессинга параметра Position исходного датасета
для приведения его к ограниченному числу меток, которые были
бы осмысленны с точки зрения модели (либо к числовому виду).

Приведены три варианта от разных участников команды: Анны,
Ярослава и Игоря.

Вариант от Анны предусматривает преобразования на основе шаблонов
регулярных выражений.

Вариант от Ярослава сначала все текстовые строки переводит в вектора
с помощью предобученной модели векторизации Word-to-Vec, затем измеряет
дистанцию между получившимися векторами и вектором слова "руководитель",
после чего эту дистанцию в виде числа от 0 до 1 записывает в соответствующую
ячейку. Алгоритм не используется в связи с большим "весом" модели
векторизации и значительно большим временем работы, нежели чем
для других вариантов.

Вариант от Игоря предусматривает преобразования на основе шаблонов. В том
числе используется в конвейере заполнения пропусков.
'''
...
def position_preproc_by_Igor(df: pd.DataFrame) -> pd.Series:
    ...
    СРЕДНЕЕ_ЗВЕНО    1302    Рядовые инженеры, врачи и специалисты с высшим
                             образованием.
    ...
    Filled = ['ДИР1', 'ДИР2', 'МЕН1', 'МЕН2', 'ИП', 'САМ',
              'ЗАМДИР', 'ЗАМНАЧ', 'НИЗШЕЕ_ЗВЕНО', 'СРЕДНЕЕ_ЗВЕНО',
              'НЕТ_ДАННЫХ', "РЕЗЕРВ"]
